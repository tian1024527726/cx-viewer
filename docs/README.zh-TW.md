...
Codex 請求監控系統，即時捕獲並視覺化展示 Codex 的所有 API 請求與回應（原始文字，不做刪減）。方便開發者監控自己的 Context，以便於 Vibe Coding 過程中回顧和排查問題。
最新版本的 CX-Viewer 還提供了伺服器部署 Web 程式設計的方案，以及行動端程式設計的工具。歡迎大家在自己的專案中應用，未來也將開放更多外掛功能，支援雲端部署。
...
cxv 是 claude 的直接替身，所有參數透傳給 claude，同時啟動 Web Viewer。

```bash
cxv                    # == claude（互動模式）
cxv -c                 # == claude --continue（繼續上次對話）
cxv -r                 # == claude --resume（恢復對話）
cxv -p "hello"         # == claude --print "hello"（列印模式）
cxv --d                # == claude --dangerously-skip-permissions（快捷方式）
...
會自動啟動一個日誌程序自動記錄請求日誌到 ~/.codex/cx-viewer/*yourproject*/date.jsonl
...
該指令會自動偵測本地 Codex 的安裝方式（NPM 或 Native Install）並進行適配。

- **NPM 版本 Codex**：自動向 Codex 的 `cli.js` 中注入攔截腳本。
- **Native 版本 Codex**：自動偵測 `claude` 二進位檔案，設定本地透明代理，並設置 Zsh Shell Hook 自動轉發流量。
- 本專案更推薦使用 npm 方式安裝的 Codex。
...
第一步：任意目錄開啟 Codex；
第二步：給 Codex 下指令，內容如下：
```
我已經安裝了 cx-viewer 這個 npm 套件，但是執行 cxv 以後仍然無法有效運行。查看 cx-viewer 的 cli.js 和 findcx.js，根據具體的環境，適配本地的 Codex 的部署方式。適配的時候修改範圍儘量約束在 findcx.js 中。
```
讓 Codex 自己檢查錯誤是比諮詢任何人以及看任何文件更有效的手段！

以上指令完成後，會更新 findcx.js。如果你的專案工程經常需要本地部署。或者 fork 出去的程式碼要經常解決安裝問題，保留這個檔案就可以。下次直接 copy 檔案。現階段很多專案和公司用 Codex 都不是 Mac 部署，而是伺服器端託管部署，所以作者剝離了 findcx.js 這個檔案，方便後續追蹤 cx-viewer 的原始碼更新。
...
如果您需要使用自訂 API 端點（例如企業代理），只需在 `~/.codex/settings.json` 中設定或設置 `OPENAI_BASE_URL` 環境變數。`cxv` 會自動識別並正確轉發請求。

### 靜默模式 (Silent Mode)

預設情況下，`cxv` 在包裹 `claude` 運行時處於靜默模式，確保您的終端輸出保持整潔，與原生體驗一致。所有日誌都在後台捕獲，並可透過 `http://localhost:7008` 查看。
...
cx-viewer 提供了客戶端版本，你可以在 GitHub 上下載到對應的客戶端版本。
[下載地址](https://github.com/weiesky/cx-viewer/releases)
目前客戶端版本在測試階段，有問題可以隨時回饋。另外 cx-viewer 的使用前提是你本地有 Codex。
需要注意的是：cx-viewer 始終只是工人（Codex）的一件「衣服」，沒有 Codex，衣服是無法獨立工作的。
...
- 即時捕獲 Codex 發出的所有 API 請求，確保是原文，而不是被刪減之後的日誌（這很重要！！！）
...
- 相容 Codex Router（CCR）及其他代理場景 — 透過 API 路徑模式兜底匹配請求
...
你可以直接自訂外掛、管理 CX-Viewer 所有程序以及 CX-Viewer 擁有對第三方介面的熱切換能力（沒錯，你可以使用 GLM、Kimi、MiniMax、Qwen、DeepSeek，雖然作者認為他們現在都很弱）
...
更多功能等你發現...比如：本系統支援 Agent Team，以及內建了 Code Reviewer。馬上就要適配 Codex 的 Code Reviewer 引入（作者很推崇使用 Codex 給 Codex Review 程式碼）


### 自動更新

CX-Viewer 啟動時自動檢查更新（每 4 小時最多一次）。同一大版本內（如 1.x.x -> 1.y.z）自動更新，下次啟動生效。跨大版本僅顯示通知提示。

自動更新跟隨 Codex 全域設定 `~/.codex/settings.json`。如果 Codex 停用了自動更新（`autoUpdates: false`），CX-Viewer 也會跳過自動更新。

### 多語言支援

CX-Viewer 支援 18 種語言，根據系統語言環境自動切換：
