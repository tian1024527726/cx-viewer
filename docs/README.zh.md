...
Codex 请求监控系统，实时捕获并可视化展示 Codex 的所有 API 请求与响应(原始文本，不做阉割)。方便开发者监控自己的 Context，以便于 Vibe Coding 过程中回顾和排查问题。
最新版本的 CX-Viewer 还提供了服务器部署web编程的方案，以及移动端编程的工具。欢迎大家在自己的项目中应用，未来也将开放更多插件功能，支持云端部署。
...
你可以在web页面里面直接使用codex，同时可以查看完整的请求报文和查看代码变更。
...
⚠️如果你仍然习惯使用codex 原生工具，或者VS code插件，请使用该模式。

这个模式下面启动 ```codex``` 或者 ```codex --dangerously-skip-permissions```

会自动启动一个日志进程自动记录请求日志到~/.codex/cx-viewer/*yourproject*/date.jsonl
...
该命令会自动检测本地 Codex 的安装方式（NPM 或 Native Install）并进行适配。

- **NPM 版本codex**：自动向 Codex 的 `cli.js` 中注入拦截脚本。
- **Native 版本 codex**：自动检测 `codex` 二进制文件，配置本地透明代理，并设置 Zsh Shell Hook 自动转发流量。
- 本项目更推荐使用 npm 方式安装的 codex。
...
第二步：给codex下指令，内容如下:
```
我已经安装了cx-viewer这个npm包，但是执行cxv以后仍然无法有效运行。查看cx-viewer的cli.js 和 findcx.js，根据具体的环境，适配本地的codex的部署方式。适配的时候修改范围尽量约束在findcx.js中。
```
让Codex自己检查错误是比咨询任何人以及看任何文档更有效的手段！

以上指令完成后，会更新findcx.js。如果你的项目工程经常需要本地部署。或者fork出去的代码要经常解决安装问题，保留这个文件就可以。下次直接copy 文件。现阶段很多项目和公司用codex都不是mac部署，而是服务端托管部署，所以作者剥离了findcx.js 这个文件，方便后续跟踪cx-viewer的源代码更新。
...
如果您需要使用自定义 API 端点（例如企业代理），只需在 `~/.codex/settings.json` 中配置或设置 `OPENAI_BASE_URL` 环境变量。`cxv` 会自动识别并正确转发请求。

### 静默模式 (Silent Mode)

默认情况下，`cxv` 在包裹 `codex` 运行时处于静默模式，确保您的终端输出保持整洁，与原生体验一致。所有日志都在后台捕获，并可通过 `http://localhost:7008` 查看。

配置完成后，正常使用 `codex` 命令即可。访问 `http://localhost:7008` 查看监控界面。


## 客户端版本

cx-viewer提供了客户端的版本，你可以在github上下载到对应客户端版本
[下载地址](https://github.com/weiesky/cx-viewer/releases)
目前客户端版本在测试阶段，有问题可以随时反馈。另外cx-viewer的使用前提是你本地有Codex。
需要注意的是：cx-viewer始终只是工人(Codex)的一件“衣服”，没有Codex，衣服是无法独立工作的。
...
- 实时捕获 Codex 发出的所有 API 请求，确保是原文，而不是被阉割之后的日志（这很重要！！！）
...
- 兼容 Codex Router（CCR）及其他代理场景 — 通过 API 路径模式兜底匹配请求
...
你可以直接自定义插件、管理cx-viewer所有进程以及cx-viewer拥有对第三方接口的热切换能力（没错，你可以使用GLM、Kimi、MiniMax、Qwen、DeepSeek，虽然作者认为他们现在都很弱）

---

<img width="1500" height="746" alt="image" src="https://github.com/user-attachments/assets/b1f60c7c-1438-4ecc-8c64-193d21ee3445" />

更多功能等你发现...比如：本系统支持Agent Team，以及内置了Code Reviewer。马上就要适配Codex 的Code Reviewer引入（作者很推崇使用Codex 给Codex Reivew 代码）


### 自动更新

CX-Viewer 启动时自动检查更新（每 4 小时最多一次）。同一大版本内（如 1.x.x → 1.y.z）自动更新，下次启动生效。跨大版本仅显示通知提示。

自动更新跟随 Codex 全局配置 `~/.codex/settings.json`。如果 Codex 禁用了自动更新（`autoUpdates: false`），CX-Viewer 也会跳过自动更新。

### 多语言支持

CX-Viewer 支持 18 种语言，根据系统语言环境自动切换：
