...
Anfragenuberwachungssystem fur Codex, das alle API-Anfragen und -Antworten in Echtzeit erfasst und visualisiert (Originaltext, ungekurzt). Ermoglicht Entwicklern die Uberwachung ihres Contexts, um Probleme beim Vibe Coding nachzuvollziehen und zu beheben.
Die neueste Version von CX-Viewer bietet ausserdem Losungen fur serverbasiertes Web-Programmieren sowie Tools fur die mobile Programmierung. Wir laden alle ein, diese in ihren eigenen Projekten einzusetzen. In Zukunft werden weitere Plugin-Funktionen und Cloud-Deployment-Unterstutzung folgen.
...
cxv ist ein direkter Ersatz fur claude -- alle Parameter werden transparent an claude weitergegeben, wahrend gleichzeitig der Web Viewer gestartet wird.

```bash
cxv                    # == claude (interaktiver Modus)
cxv -c                 # == claude --continue (letzte Konversation fortsetzen)
cxv -r                 # == claude --resume (Konversation wiederherstellen)
cxv -p "hello"         # == claude --print "hello" (Druckmodus)
cxv --d                # == claude --dangerously-skip-permissions (Kurzform)
...
automatisch ein Log-Prozess gestartet, der Anfragen in ~/.codex/cx-viewer/*IhrProjekt*/date.jsonl protokolliert.
...
Dieser Befehl erkennt automatisch die Installationsmethode von Codex (NPM oder Native Install) und passt sich entsprechend an.

- **NPM-Version von Codex**: Injiziert automatisch das Interceptor-Skript in `cli.js` von Codex.
- **Native-Version von Codex**: Erkennt automatisch die `claude`-Binardatei, konfiguriert einen lokalen transparenten Proxy und richtet einen Zsh Shell Hook ein, um den Datenverkehr automatisch weiterzuleiten.
- Dieses Projekt empfiehlt die Verwendung der uber NPM installierten Version von Codex.
...
Schritt 1: Offnen Sie Codex in einem beliebigen Verzeichnis;
Schritt 2: Geben Sie Codex folgende Anweisung:
```
Ich habe das npm-Paket cx-viewer installiert, aber nach dem Ausfuhren von cxv funktioniert es immer noch nicht richtig. Schau dir cli.js und findcx.js von cx-viewer an und passe die lokale Codex-Deployment-Methode entsprechend der konkreten Umgebung an. Beschranke die Anderungen dabei moglichst auf findcx.js.
```
Codex selbst die Fehler prufen zu lassen ist effektiver als jede andere Person zu fragen oder Dokumentation zu lesen!

Nachdem die obigen Anweisungen abgeschlossen sind, wird findcx.js aktualisiert. Wenn Ihr Projekt haufig lokal bereitgestellt werden muss oder der geforkte Code regelmasig Installationsprobleme losen muss, behalten Sie diese Datei einfach bei. Beim nachsten Mal konnen Sie sie direkt kopieren. Derzeit setzen viele Projekte und Unternehmen Codex nicht auf dem Mac ein, sondern auf serverseitig gehosteten Deployments. Daher hat der Autor findcx.js ausgelagert, um das Nachverfolgen von cx-viewer-Quellcode-Updates zu erleichtern.
...
Wenn Sie einen benutzerdefinierten API-Endpunkt verwenden mussen (z. B. Unternehmens-Proxy), konfigurieren Sie ihn einfach in `~/.codex/settings.json` oder setzen Sie die Umgebungsvariable `OPENAI_BASE_URL`. `cxv` erkennt diese Einstellungen automatisch und leitet Anfragen korrekt weiter.

### Stiller Modus (Silent Mode)

Standardmasig lauft `cxv` beim Wrappen von `claude` im stillen Modus, um sicherzustellen, dass Ihre Terminalausgabe sauber bleibt und identisch mit der nativen Erfahrung ist. Alle Protokolle werden im Hintergrund erfasst und sind unter `http://localhost:7008` einsehbar.
...
CX-Viewer bietet eine Desktop-Client-Version an, die Sie auf GitHub herunterladen konnen:
[Download-Link](https://github.com/weiesky/cx-viewer/releases)
Die Client-Version befindet sich derzeit in der Testphase. Bei Problemen konnen Sie jederzeit Feedback geben. Ausserdem setzt die Nutzung von cx-viewer voraus, dass Codex lokal installiert ist.
Bitte beachten Sie: cx-viewer ist lediglich ein „Kleidungsstuck" fur den Arbeiter (Codex). Ohne Codex kann das Kleidungsstuck nicht eigenstandig funktionieren.
...
Nach dem Start mit cxv sehen Sie:
...
### Logger-Modus (vollstandige Codex Konversation anzeigen)

<img width="1500" height="768" alt="image" src="https://github.com/user-attachments/assets/a8a9f3f7-d876-4f6b-a64d-f323a05c4d21" />


- Erfasst in Echtzeit alle von Codex gesendeten API-Anfragen und stellt sicher, dass es sich um den Originaltext handelt -- nicht um gekurzte Logs (das ist sehr wichtig!!!)
...
- Kompatibel mit Codex Router (CCR) und anderen Proxy-Szenarien -- Anfragen werden uber API-Pfadmuster-Fallback abgeglichen
...
Uber das CX-Viewer-Dropdown-Menu oben links:
...
Sie konnen eigene Plugins erstellen, alle CX-Viewer-Prozesse verwalten und CX-Viewer bietet Hot-Switching fur Drittanbieter-Schnittstellen (ja, Sie konnen GLM, Kimi, MiniMax, Qwen, DeepSeek verwenden -- auch wenn der Autor findet, dass sie derzeit noch recht schwach sind)
...
Weitere Funktionen warten darauf, entdeckt zu werden... zum Beispiel: Das System unterstutzt Agent Team und enthalt einen integrierten Code Reviewer. Die Integration des Codex Code Reviewers steht kurz bevor (der Autor empfiehlt sehr, Codex fur Codex Reviews einzusetzen)


### Automatische Updates

CX-Viewer pruft beim Start automatisch auf Updates (maximal einmal alle 4 Stunden). Innerhalb derselben Hauptversion (z. B. 1.x.x -> 1.y.z) wird automatisch aktualisiert und beim nachsten Start wirksam. Bei einem Hauptversionswechsel wird nur eine Benachrichtigung angezeigt.

Die automatische Aktualisierung folgt der globalen Codex-Konfiguration `~/.codex/settings.json`. Wenn Codex die automatischen Updates deaktiviert hat (`autoUpdates: false`), uberspringt CX-Viewer die automatische Aktualisierung ebenfalls.

### Mehrsprachige Unterstutzung

CX-Viewer unterstutzt 18 Sprachen und wechselt automatisch basierend auf der Systemsprache:
