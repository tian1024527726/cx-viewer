...
System monitorowania żądań Codex — przechwytuje i wizualizuje w czasie rzeczywistym wszystkie żądania i odpowiedzi API Codex (oryginalny tekst, bez cenzury). Pomaga programistom monitorować własny Context, ułatwiając przegląd i diagnozowanie problemów podczas Vibe Coding.
Najnowsza wersja CX-Viewer oferuje również rozwiązanie do programowania webowego na serwerze oraz narzędzia do programowania mobilnego. Zapraszamy do stosowania w swoich projektach — w przyszłości zostanie udostępnionych więcej funkcji wtyczek z obsługą wdrożeń w chmurze.
...
cxv jest bezpośrednim zamiennikiem claude — wszystkie argumenty są przekazywane do claude, a jednocześnie uruchamiany jest Web Viewer.

```bash
cxv                    # == claude (tryb interaktywny)
cxv -c                 # == claude --continue (kontynuuj ostatnią rozmowę)
cxv -r                 # == claude --resume (wznów rozmowę)
cxv -p "hello"         # == claude --print "hello" (tryb drukowania)
cxv --d                # == claude --dangerously-skip-permissions (skrót)
...
Automatycznie zostanie uruchomiony proces logowania, który zapisuje logi żądań do ~/.codex/cx-viewer/*twojprojekt*/date.jsonl
...
Polecenie automatycznie wykrywa sposób instalacji Codex (NPM lub Native Install) i odpowiednio się dostosowuje.

- **Wersja NPM claude code**: automatycznie wstrzykuje skrypt przechwytujący do `cli.js` Codex.
...
```
Zainstalowałem pakiet npm cx-viewer, ale po uruchomieniu cxv nadal nie działa poprawnie. Sprawdź cli.js i findcx.js w cx-viewer i dostosuj je do lokalnego sposobu wdrożenia claude code w tym środowisku. Ogranicz zakres zmian do pliku findcx.js.
```
Pozwolenie Codex na samodzielne sprawdzenie błędów jest skuteczniejsze niż konsultowanie się z kimkolwiek lub czytanie dokumentacji!

Po wykonaniu powyższego polecenia plik findcx.js zostanie zaktualizowany. Jeśli Twój projekt często wymaga lokalnego wdrożenia lub rozwidlony kod często wymaga rozwiązywania problemów instalacyjnych, zachowaj ten plik — następnym razem wystarczy go skopiować. Ponieważ wiele projektów i firm używa claude code na serwerach, a nie na Macu, autor wyodrębnił plik findcx.js, aby ułatwić śledzenie aktualizacji kodu źródłowego cx-viewer.
...
Jeśli potrzebujesz użyć niestandardowego punktu końcowego API (np. proxy firmowego), skonfiguruj go w `~/.codex/settings.json` lub ustaw zmienną środowiskową `OPENAI_BASE_URL`. `cxv` automatycznie to rozpozna i poprawnie przekaże żądania.

### Tryb cichy (Silent Mode)

Domyślnie `cxv` działa w trybie cichym podczas owijania `claude`, zapewniając czysty wynik terminala zgodny z natywnym doświadczeniem. Wszystkie logi są przechwytywane w tle i dostępne pod adresem `http://localhost:7008`.
...
CX-Viewer jest dostepny jako aplikacja kliencka, ktora mozna pobrac z GitHuba.
[Pobierz tutaj](https://github.com/weiesky/cx-viewer/releases)
Obecnie wersja kliencka jest w fazie testowania — w razie problemow mozesz w kazdej chwili zglosic uwagi. Ponadto korzystanie z cx-viewer wymaga zainstalowanego lokalnie Codex.
Nalezy pamietac: cx-viewer to jedynie „ubranie" dla pracownika (Codex). Bez Codex ubranie nie moze dzialac samodzielnie.
...
Po uruchomieniu za pomocą cxv zobaczysz:
...
- Przechwytuje w czasie rzeczywistym wszystkie żądania API wysyłane przez Codex — zapewnia oryginalny tekst, a nie ocenzurowane logi (to bardzo ważne!!!)
...
- Kompatybilny z Codex Router (CCR) i innymi scenariuszami proxy — dopasowuje żądania na podstawie wzorca ścieżki API
...
Przez menu rozwijane CX-Viewer w lewym górnym rogu:
...
Możesz bezpośrednio dostosowywać wtyczki, zarządzać wszystkimi procesami CX-Viewer, a CX-Viewer oferuje możliwość hot-swap interfejsów firm trzecich (tak, możesz używać GLM, Kimi, MiniMax, Qwen, DeepSeek — choć autor uważa, że obecnie są one dość słabe)
...
Więcej funkcji czeka na odkrycie... np.: System obsługuje Agent Team i ma wbudowany Code Reviewer. Integracja z Codex Code Reviewer jest tuż za rogiem (autor gorąco poleca używanie Codex do code review kodu Codex)


### Automatyczne aktualizacje

CX-Viewer automatycznie sprawdza aktualizacje przy uruchomieniu (maksymalnie raz na 4 godziny). W ramach tej samej głównej wersji (np. 1.x.x → 1.y.z) aktualizacje są stosowane automatycznie i wchodzą w życie przy następnym uruchomieniu. Aktualizacje między głównymi wersjami wyświetlają tylko powiadomienie.

Automatyczne aktualizacje są zgodne z globalną konfiguracją Codex `~/.codex/settings.json`. Jeśli Codex ma wyłączone automatyczne aktualizacje (`autoUpdates: false`), CX-Viewer również pominie automatyczne aktualizacje.

### Obsługa wielu języków

CX-Viewer obsługuje 18 języków i automatycznie przełącza się na podstawie języka systemu:
