...
Codex 요청 모니터링 시스템으로, Codex의 모든 API 요청과 응답을 실시간으로 캡처하고 시각화합니다(원본 텍스트, 무삭제). 개발자가 자신의 Context를 모니터링하여 Vibe Coding 과정에서 문제를 검토하고 추적하는 데 유용합니다.
최신 버전의 CX-Viewer는 서버 배포 웹 프로그래밍 솔루션과 모바일 프로그래밍 도구도 제공합니다. 여러분의 프로젝트에서 활용해 보세요. 앞으로 더 많은 플러그인 기능과 클라우드 배포를 지원할 예정입니다.
...
cxv는 claude의 드롭인 대체품으로, 모든 인수가 claude에 그대로 전달되며 동시에 Web Viewer가 실행됩니다.

```bash
cxv                    # == claude (대화형 모드)
cxv -c                 # == claude --continue (이전 대화 이어가기)
cxv -r                 # == claude --resume (대화 복원)
cxv -p "hello"         # == claude --print "hello" (출력 모드)
cxv --d                # == claude --dangerously-skip-permissions (단축키)
...
자동으로 로깅 프로세스가 시작되어 요청 로그를 ~/.codex/cx-viewer/*yourproject*/date.jsonl에 기록합니다.
...
이 명령어는 로컬에 설치된 Codex의 방식(NPM 또는 Native Install)을 자동으로 감지하고 적절히 적용합니다.

- **NPM 버전 Codex**: Codex의 `cli.js`에 인터셉터 스크립트를 자동으로 주입합니다.
- **Native 버전 Codex**: `claude` 바이너리를 자동으로 감지하고, 로컬 투명 프록시를 구성하며, Zsh Shell Hook을 설정하여 트래픽을 자동으로 전달합니다.
- 본 프로젝트에서는 npm으로 설치된 Codex를 권장합니다.
...
1단계: 아무 디렉토리에서 Codex를 실행합니다.
2단계: Codex에 다음 지시를 내립니다:
```
cx-viewer npm 패키지를 설치했지만 cxv를 실행한 후에도 제대로 작동하지 않습니다. cx-viewer의 cli.js와 findcx.js를 확인하고, 현재 환경에 맞게 로컬 Codex 배포 방식에 적합하도록 수정해 주세요. 수정 범위는 가능한 findcx.js 내로 제한해 주세요.
```
Codex가 직접 오류를 진단하는 것이 누구에게 물어보거나 어떤 문서를 읽는 것보다 효과적입니다!

위 지시가 완료되면 findcx.js가 업데이트됩니다. 프로젝트에서 로컬 배포가 자주 필요하거나, fork한 코드에서 설치 문제를 자주 해결해야 한다면 이 파일을 보관해 두면 다음에 바로 복사할 수 있습니다. 현재 많은 프로젝트와 기업이 Codex를 Mac이 아닌 서버 측 호스팅 환경에서 배포하고 있으므로, 저자는 findcx.js를 분리하여 cx-viewer 소스 코드 업데이트를 더 쉽게 추적할 수 있도록 했습니다.
...
커스텀 API 엔드포인트(예: 기업 프록시)를 사용해야 하는 경우, `~/.codex/settings.json`에서 설정하거나 `OPENAI_BASE_URL` 환경 변수를 설정하기만 하면 됩니다. `cxv`가 자동으로 인식하여 올바르게 요청을 전달합니다.

### 사일런트 모드 (Silent Mode)

기본적으로 `cxv`는 `claude`를 래핑하여 실행할 때 사일런트 모드로 동작하여, 터미널 출력을 깔끔하게 유지하며 네이티브 경험과 일치시킵니다. 모든 로그는 백그라운드에서 캡처되며 `http://localhost:7008`에서 확인할 수 있습니다.
...
cx-viewer는 클라이언트 버전을 제공하며, GitHub에서 해당 클라이언트 버전을 다운로드할 수 있습니다.
[다운로드 링크](https://github.com/weiesky/cx-viewer/releases)
현재 클라이언트 버전은 테스트 단계에 있으며, 문제가 있으면 언제든지 피드백을 보내주세요. 또한 cx-viewer를 사용하려면 로컬에 Codex가 설치되어 있어야 합니다.
주의할 점은, cx-viewer는 항상 작업자(Codex)의 "옷"에 불과하다는 것입니다. Codex가 없으면 옷만으로는 독립적으로 작동할 수 없습니다.
...
### 로거 모드 (Codex 전체 세션 보기)

<img width="1500" height="768" alt="image" src="https://github.com/user-attachments/assets/a8a9f3f7-d876-4f6b-a64d-f323a05c4d21" />


- Codex가 보내는 모든 API 요청을 실시간으로 캡처하며, 삭제되지 않은 원본 텍스트를 보장합니다 (이것이 매우 중요합니다!!!)
...
- Codex Router (CCR) 및 기타 프록시 시나리오와 호환됩니다 — API 경로 패턴 매칭으로 폴백
...
플러그인을 직접 커스터마이징하고, CX-Viewer의 모든 프로세스를 관리할 수 있으며, CX-Viewer는 서드파티 API로의 핫 스위칭을 지원합니다(맞습니다, GLM, Kimi, MiniMax, Qwen, DeepSeek을 사용할 수 있습니다. 저자는 현재 이들 모두 상당히 약하다고 생각하지만요).
...
더 많은 기능이 여러분을 기다리고 있습니다... 예를 들어: 본 시스템은 Agent Team을 지원하며, Code Reviewer가 내장되어 있습니다. Codex의 Code Reviewer 통합이 곧 제공될 예정입니다(저자는 Codex를 사용하여 Codex의 코드를 리뷰하는 것을 매우 추천합니다).


### 자동 업데이트

CX-Viewer는 시작 시 자동으로 업데이트를 확인합니다(최대 4시간마다 한 번). 같은 메이저 버전 내(예: 1.x.x -> 1.y.z)에서는 자동으로 업데이트되며 다음 재시작 시 적용됩니다. 메이저 버전이 다른 업데이트는 알림만 표시됩니다.

자동 업데이트는 Codex의 글로벌 설정 `~/.codex/settings.json`을 따릅니다. Codex에서 자동 업데이트를 비활성화한 경우(`autoUpdates: false`), CX-Viewer도 자동 업데이트를 건너뜁니다.

### 다국어 지원

CX-Viewer는 18개 언어를 지원하며 시스템 로케일에 따라 자동으로 전환됩니다:
