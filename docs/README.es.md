...
Sistema de monitoreo de solicitudes para Codex que captura y visualiza en tiempo real todas las solicitudes y respuestas de API (texto original, sin recortes). Permite a los desarrolladores monitorear su Context para revisar y solucionar problemas durante el Vibe Coding.
La ultima version de CX-Viewer tambien ofrece soluciones de programacion web con despliegue en servidor, asi como herramientas de programacion para dispositivos moviles. Te invitamos a utilizarlo en tus propios proyectos; en el futuro se abriran mas funciones de plugins y se admitira el despliegue en la nube.
...
cxv es un reemplazo directo de claude -- todos los parametros se pasan directamente a claude, mientras se inicia el Web Viewer.

```bash
cxv                    # == claude (modo interactivo)
cxv -c                 # == claude --continue (continuar ultima conversacion)
cxv -r                 # == claude --resume (reanudar conversacion)
cxv -p "hello"         # == claude --print "hello" (modo impresion)
cxv --d                # == claude --dangerously-skip-permissions (atajo)
...
se iniciara automaticamente un proceso de registro que guarda los logs de solicitudes en ~/.codex/cx-viewer/*tuproyecto*/date.jsonl
...
Este comando detecta automaticamente el metodo de instalacion local de Codex (NPM o Native Install) y se adapta en consecuencia.

- **Codex version NPM**: Inyecta automaticamente el script interceptor en `cli.js` de Codex.
- **Codex version Native**: Detecta automaticamente el binario `claude`, configura un proxy transparente local y establece un Zsh Shell Hook para redirigir el trafico automaticamente.
- Se recomienda usar Codex instalado mediante NPM para este proyecto.
...
Paso 1: abre Codex en cualquier directorio;
Paso 2: dale a Codex la siguiente instruccion:
```
He instalado el paquete npm cx-viewer, pero despues de ejecutar cxv sigue sin funcionar correctamente. Revisa cli.js y findcx.js de cx-viewer, y adapta la implementacion local de Codex segun el entorno especifico. Intenta limitar el alcance de los cambios a findcx.js.
```
Dejar que Codex inspeccione los errores por si mismo es mas efectivo que preguntarle a cualquier persona o leer cualquier documentacion!

Una vez completada la instruccion anterior, se actualizara findcx.js. Si tu proyecto frecuentemente requiere despliegue local, o si el codigo bifurcado necesita resolver problemas de instalacion con frecuencia, conserva este archivo para copiarlo directamente la proxima vez. En la actualidad, muchos proyectos y empresas que usan Codex no lo despliegan en Mac sino en servidores, por lo que el autor separo el archivo findcx.js para facilitar el seguimiento de las actualizaciones del codigo fuente de cx-viewer.
...
Si necesitas utilizar un endpoint de API personalizado (por ejemplo, un proxy corporativo), simplemente configuralo en `~/.codex/settings.json` o establece la variable de entorno `OPENAI_BASE_URL`. `cxv` reconocera automaticamente esta configuracion y reenviara las solicitudes correctamente.

### Modo silencioso (Silent Mode)

De forma predeterminada, `cxv` se ejecuta en modo silencioso al envolver `claude`, lo que garantiza que la salida de tu terminal permanezca limpia e identica a la experiencia nativa. Todos los registros se capturan en segundo plano y son visibles en `http://localhost:7008`.
...
cx-viewer ofrece una version de escritorio que puedes descargar desde GitHub.
[Enlace de descarga](https://github.com/weiesky/cx-viewer/releases)
Actualmente la version de escritorio se encuentra en fase de pruebas; si encuentras algun problema, no dudes en enviarnos tus comentarios. Ademas, el requisito previo para usar cx-viewer es tener Codex instalado localmente.
Es importante tener en cuenta que cx-viewer es simplemente una "ropa" para el trabajador (Codex). Sin Codex, la ropa no puede funcionar por si sola.
...
Despues de iniciar con cxv puedes ver:
...
### Modo logger (ver conversacion completa de Codex)

<img width="1500" height="768" alt="image" src="https://github.com/user-attachments/assets/a8a9f3f7-d876-4f6b-a64d-f323a05c4d21" />


- Captura en tiempo real todas las solicitudes de API enviadas por Codex, asegurando que sea el texto original, no logs recortados (esto es muy importante!!!)
...
- Compatible con Codex Router (CCR) y otros escenarios de proxy -- coincidencia de solicitudes mediante patron de ruta API como respaldo
...
A traves del menu desplegable de CX-Viewer en la esquina superior izquierda:
...
Puedes crear plugins personalizados, gestionar todos los procesos de CX-Viewer y CX-Viewer ofrece cambio en caliente de interfaces de terceros (si, puedes usar GLM, Kimi, MiniMax, Qwen, DeepSeek -- aunque el autor opina que actualmente son bastante debiles)
...
Mas funciones esperan ser descubiertas... por ejemplo: el sistema soporta Agent Team e incluye un Code Reviewer integrado. La integracion del Code Reviewer de Codex esta por llegar (el autor recomienda mucho usar Codex para hacer Code Review de Codex)


### Actualizacion automatica

CX-Viewer comprueba automaticamente si hay actualizaciones al iniciar (como maximo una vez cada 4 horas). Dentro de la misma version mayor (por ejemplo, 1.x.x -> 1.y.z) se actualiza automaticamente y surte efecto en el siguiente inicio. Para cambios de version mayor solo se muestra una notificacion.

La actualizacion automatica sigue la configuracion global de Codex en `~/.codex/settings.json`. Si Codex tiene desactivadas las actualizaciones automaticas (`autoUpdates: false`), CX-Viewer tambien omitira la actualizacion automatica.

### Soporte multilingue

CX-Viewer admite 18 idiomas y cambia automaticamente segun el idioma del sistema:
