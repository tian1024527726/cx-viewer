...
Systeme de surveillance des requetes Codex, capturant et visualisant en temps reel toutes les requetes et reponses API (texte brut, sans troncature). Permet aux developpeurs de surveiller leur Context afin de faciliter la revue et le diagnostic lors du Vibe Coding.
La derniere version de CX-Viewer propose egalement des solutions de deploiement serveur pour la programmation web, ainsi que des outils de programmation mobile. N'hesitez pas a l'utiliser dans vos projets ; davantage de fonctionnalites de plugins seront disponibles a l'avenir, avec prise en charge du deploiement cloud.
...
cxv est un substitut direct de claude -- tous les parametres sont transmis a claude, tout en demarrant le Web Viewer.

```bash
cxv                    # == claude (mode interactif)
cxv -c                 # == claude --continue (continuer la derniere conversation)
cxv -r                 # == claude --resume (reprendre la conversation)
cxv -p "hello"         # == claude --print "hello" (mode impression)
cxv --d                # == claude --dangerously-skip-permissions (raccourci)
...
demarre automatiquement un processus de journalisation qui enregistre les journaux de requetes dans ~/.codex/cx-viewer/*votreprojet*/date.jsonl
...
Cette commande detecte automatiquement le mode d'installation local de Codex (NPM ou Native Install) et s'adapte en consequence.

- **Version NPM de Codex** : injecte automatiquement un script d'interception dans le fichier `cli.js` de Codex.
- **Version Native de Codex** : detecte automatiquement le binaire `claude`, configure un proxy transparent local et met en place un Zsh Shell Hook pour rediriger le trafic automatiquement.
- Ce projet recommande l'utilisation de Codex installe via NPM.
...
Etape 1 : ouvrez Codex dans n'importe quel repertoire ;
Etape 2 : donnez l'instruction suivante a Codex :
```
J'ai installe le package npm cx-viewer, mais apres avoir execute cxv, il ne fonctionne toujours pas correctement. Examine cli.js et findcx.js de cx-viewer, puis adapte-le au mode de deploiement local de Codex en fonction de l'environnement specifique. Limite autant que possible les modifications au fichier findcx.js.
```
Laisser Codex diagnostiquer lui-meme les erreurs est plus efficace que de consulter qui que ce soit ou de lire n'importe quelle documentation !

Une fois l'instruction executee, findcx.js sera mis a jour. Si votre projet necessite frequemment un deploiement local, ou si le code forke doit souvent resoudre des problemes d'installation, conservez ce fichier -- vous pourrez simplement le copier la prochaine fois. A l'heure actuelle, de nombreux projets et entreprises n'utilisent pas Codex sur Mac mais via un deploiement serveur heberge, c'est pourquoi l'auteur a separe le fichier findcx.js pour faciliter le suivi des mises a jour du code source de cx-viewer.
...
Si vous devez utiliser un point de terminaison API personnalise (par exemple un proxy d'entreprise), il suffit de le configurer dans `~/.codex/settings.json` ou de definir la variable d'environnement `OPENAI_BASE_URL`. `cxv` le detectera automatiquement et transmettra correctement les requetes.

### Mode silencieux (Silent Mode)

Par defaut, `cxv` fonctionne en mode silencieux lorsqu'il encapsule `claude`, garantissant que la sortie de votre terminal reste propre et coherente avec l'experience native. Tous les journaux sont captures en arriere-plan et consultables via `http://localhost:7008`.
...
CX-Viewer propose une version client de bureau que vous pouvez telecharger depuis GitHub :
[Lien de telechargement](https://github.com/weiesky/cx-viewer/releases)
La version client est actuellement en phase de test ; n'hesitez pas a nous faire part de vos retours en cas de probleme. Par ailleurs, l'utilisation de cx-viewer necessite que Codex soit installe localement.
Il est important de noter que cx-viewer n'est qu'un « vetement » pour l'ouvrier (Codex). Sans Codex, le vetement ne peut pas fonctionner seul.
...
Apres avoir lance cxv, vous pouvez voir :
...
### Mode journal (voir la session complete de Codex)

<img width="1500" height="768" alt="image" src="https://github.com/user-attachments/assets/a8a9f3f7-d876-4f6b-a64d-f323a05c4d21" />


- Capture en temps reel de toutes les requetes API emises par Codex, garantissant le texte original, et non des journaux tronques (c'est tres important !!!)
...
- Compatible avec Codex Router (CCR) et autres scenarios de proxy -- correspondance de secours des requetes via le modele de chemin API
...
Via le menu deroulant CX-Viewer en haut a gauche :
...
Vous pouvez creer des plugins personnalises, gerer tous les processus de CX-Viewer et CX-Viewer offre le basculement a chaud d'interfaces tierces (oui, vous pouvez utiliser GLM, Kimi, MiniMax, Qwen, DeepSeek -- bien que l'auteur estime qu'ils sont encore assez faibles actuellement)
...
D'autres fonctionnalites attendent d'etre decouvertes... par exemple : le systeme prend en charge Agent Team et integre un Code Reviewer. L'integration du Code Reviewer de Codex arrive bientot (l'auteur recommande vivement d'utiliser Codex pour faire du Code Review sur Codex)


### Mise a jour automatique

CX-Viewer verifie automatiquement les mises a jour au demarrage (une fois toutes les 4 heures maximum). Les mises a jour au sein d'une meme version majeure (par exemple 1.x.x -> 1.y.z) sont appliquees automatiquement et prennent effet au prochain demarrage. Les changements de version majeure affichent uniquement une notification.

La mise a jour automatique suit la configuration globale de Codex `~/.codex/settings.json`. Si Codex a desactive les mises a jour automatiques (`autoUpdates: false`), CX-Viewer ignorera egalement la mise a jour automatique.

### Support multilingue

CX-Viewer prend en charge 18 langues, avec basculement automatique selon la langue du systeme :
